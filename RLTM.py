...
# Slots we expose. Hidden from the user: SeasonLogos, AITeams, Crates, etc.
# (those aren't really "swappable cosmetics" the user thinks about).
SUPPORTED_SLOTS = [
    "Decal",
    "Wheels",
    "Rocket Boost",
...
# Slots known to crash when swapped (per the README warnings).
...
class AutocompleteCombobox(ttk.Entry):
    ...
    def _on_entry_click(self, event):
        # Open the full list as soon as you click the empty box
    ...
    def _filter_and_show(self):
        ...
        if text == '':
            ...
            # Filter the list in the background
            hits = [item for item in self._completion_list if text.lower() in item.lower()]
    ...
    def _show_list(self, hits):
        ...
        # Populate the list with the filtered items
        for item in hits:
    ...
    def _on_list_click(self, event):
        # When an item is clicked, put it in the text box and close the menu
        if self._listbox:
    ...
    def _on_focusout(self, event):
        # A tiny delay so the click actually registers before the list vanishes
        self.after(150, self._close_list)

# Quick-swap presets. (target_id, donor_id, label).
# IDs from items.json: Standard=63, Sparkles=62, Alpha (Gold Rush)=32.
PRESETS = [
    (63, 32, "Standard Boost → Alpha Boost"),
    (62, 32, "Sparkles → Alpha Boost"),
]
...
class ModderApp:
    ...
    def _build_ui(self) -> None:
        ...
        style.configure("Preset.TButton", font=("Segoe UI", 9), padding=(14, 8))
        ...
        # Quick-swap presets
        ttk.Label(outer, text="Quick swaps", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        ttk.Label(
            outer,
            text="One-click presets — applies immediately.",
            style="Sub.TLabel",
        ).pack(anchor="w", pady=(0, 4))
        preset_row = ttk.Frame(outer)
        preset_row.pack(fill="x", pady=(0, 14))
        for target_id, donor_id, label in PRESETS:
            btn = ttk.Button(
                preset_row,
                text=label,
                style="Preset.TButton",
                command=lambda t=target_id, d=donor_id, l=label: self._apply_preset(t, d, l),
            )
        ...
        ttk.Label(
            outer,
            text="Pick a slot, then choose what to replace and what to replace it with.",
            style="Sub.TLabel",
        ).pack(anchor="w", pady=(0, 6))

        # Slot picker
        slot_row = ttk.Frame(outer)
        slot_row.pack(fill="x", pady=(0, 8))
        ...
        self.risk_label.pack(anchor="w", pady=(2, 4))
        ...
        self.target_combo = AutocompleteCombobox(picker, textvariable=self.target_var) # Changed and readonly removed
        ...
        self.donor_combo = AutocompleteCombobox(picker, textvariable=self.donor_var) # Changed and readonly removed
        ...
        ttk.Button(action_row, text="Revert selected", command=self._revert_selected).pack(side="left")
        ttk.Button(action_row, text="Clear list", command=self._clear_active_list).pack(side="left", padx=(8, 0))
    ...
    def _load_items(self) -> None:
        ...
        # Bucket by slot, only for supported slots
        for slot in SUPPORTED_SLOTS:
            self.slot_items[slot] = sorted(
                [it for it in self.items if it.slot == slot],
    ...
    def _on_slot_change(self, *_) -> None:
        ...
        if slot in RISKY_SLOTS:
            self.risk_label.config(
                text=f"⚠ {slot} swaps sometimes crash the game on load. Use Revert if it happens."
            )
        else:
    ...
    def _clear_active_list(self) -> None:
        ...
        if messagebox.askyesno(
            APP_NAME,
            "Clear the list of active swaps?\n\nThis only clears the tracking list — it does NOT revert the swaps in your game. Use Steam/Epic 'verify game files' to actually reset.",
        ):
            self.active_swaps = []
    ...
    def _revert_selected(self) -> None:
        ...
        if not sel:
            messagebox.showinfo(APP_NAME, "Pick a swap from the list first.")
            return
        ...
        if backup.exists():
            try:
                ...
                messagebox.showinfo(
                    APP_NAME,
                    f"{swap.target_product} reverted successfully.\n\nRestart Rocket League to see the change.",
                )
            except Exception as exc:
    ...
    def _apply_preset(self, target_id: int, donor_id: int, label: str) -> None:
        target = self._resolve_id_to_item(target_id)
        donor = self._resolve_id_to_item(donor_id)
        if not target or not donor:
            messagebox.showerror(APP_NAME, f"Preset items missing from items.json:\n{label}")
            return
    ...
    def _apply_custom_swap(self) -> None:
        ...
        if not target or not donor:
            messagebox.showwarning(APP_NAME, "Pick both items first.")
            return
        if target.id == donor.id:
            messagebox.showinfo(APP_NAME, "Pick two different items.")
            return
        self._run_swap(target, donor)
    ...
    def _on_swap_success(self, target, donor) -> None:
        ...
        messagebox.showinfo(
            APP_NAME,
            f"Done! Your {target.product} will now look like {donor.product}.\n\n"
            f"Restart Rocket League to see the change.",
        )
...
def main() -> int:
    ...
    try:
        # Hide the icon for now — set one if you've got an .ico
        root.iconbitmap(default=str(app_dir() / "icon.ico")) if (app_dir() / "icon.ico").exists() else None
    except Exception:
        pass
